...
**Q: Port 27017 or 6379 is already in use**
...
- A: Ensure MongoDB is running via Docker (`docker-compose up -d mongodb`) or locally. Verify the `MONGODB_URI` in your `.env` file is correct.

## 🧪 Testing

**Q: Tests are failing with database connection errors**

- A: Run the Docker services (`docker-compose up -d mongodb redis`) before executing `pnpm test`.
